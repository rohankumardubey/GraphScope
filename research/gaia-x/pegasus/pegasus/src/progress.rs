...
//                 }
//             }
//             (PeerSet::Single(a), PeerSet::Partial(b)) => {
//                 if b.contains(a) {
//                     if b.len() == 1 {
//                         Some(Ordering::Equal)
//                     } else {
...
//                 }
//             }
//             (PeerSet::Single(_), PeerSet::All(_)) => Some(Ordering::Less),
//             (PeerSet::Partial(a), PeerSet::Single(b)) => {
//                 if a.contains(b) {
//                     if a.len() == 1 {
//                         Some(Ordering::Equal)
//                     } else {
...
//                 }
//             }
//             (PeerSet::All(_), PeerSet::Single(_)) => Some(Ordering::Greater),
//             (PeerSet::All(a), PeerSet::Partial(b)) => {
//                 if *a == b.len() as u32 {
//                     for i in 0..*a {
//                         if !b.contains(&i) {
...
impl DynPeers {
    ...
    pub fn add_source(&mut self, worker: u32) {
        ...
        match mask {
            PeerSet::None => {
                self.mask = PeerSet::Single(worker)
            },
            PeerSet::Single(a) => {
...
impl Debug for DynPeers {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self.mask {
            PeerSet::None => write!(f, "[]"),
            PeerSet::Single(x) => write!(f, "[{}]", x),
            PeerSet::Partial(ref p) => write!(f, "{:?}", p),
